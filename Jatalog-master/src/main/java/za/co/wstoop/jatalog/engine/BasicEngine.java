...
public class BasicEngine extends Engine {
    ...
    private Collection<Expr> expandStrata(IndexedSet<Expr,String> facts, Collection<Rule> strataRules) {
        ...
        //Map<String, Collection<Rule>> dependentNewRules = buildDependentRules(strataRules);


        IndexedSet<Expr,String> qFacts = new IndexedSet<>();
        ...
        while(true) {
            ...
            //IndexedSet<Expr,String> newFacts = new IndexedSet<>();
            qFacts.addAll(newFacts);
            for(Rule rule : rules) {
                newFacts.addAll(matchRule(facts, rule));
            }
           // oldFacts.removeAll(facts);
            newFacts.removeAll(facts);
            if(newFacts.isEmpty()) {
                return facts;
            }
    ...
    /*Magic set*/
    private Collection<Expr> naive(IndexedSet<Expr,String> facts, Collection<Rule> strataRules){
        if (strataRules == null || strataRules.isEmpty()) {
            return Collections.emptyList();
        }
